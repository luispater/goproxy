...
var (
	...
	WaitForAuthConnect = &ConnectAction{Action: ConnectWaitForAuth, TLSConfig: TLSConfigFromCA(&GoproxyCa)}
...
func (proxy *ProxyHttpServer) handleHttps(w http.ResponseWriter, r *http.Request) {
	...
	case ConnectReject:
		...
		proxyClient.Close()
	case ConnectWaitForAuth:
		if ctx.Resp != nil {
			if err := ctx.Resp.Write(proxyClient); err != nil {
				ctx.Warnf("Wait for auth http CONNECT: %v", err)
			}
		}
		client := bufio.NewReader(proxyClient)
		req, err := http.ReadRequest(client)
		...
		if req != nil {
			...
			proxy.handleHttpsHijacked(w, req, proxyClient)
		}
	}
}

func (proxy *ProxyHttpServer) handleHttpsHijacked(w http.ResponseWriter, r *http.Request, proxyClient net.Conn) {
	ctx := &ProxyCtx{Req: r, Session: atomic.AddInt64(&proxy.sess, 1), proxy: proxy}
